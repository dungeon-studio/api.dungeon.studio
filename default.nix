...
, siren-json, stdenv, test-invariant, text, time, transformers
, unordered-containers, uuid, wai, wai-logger, warp
}:
mkDerivation {
  ...
  testHaskellDepends = [
    ...
    mtl network-uri network-uri-json process QuickCheck
    quickcheck-instances regex-compat servant servant-server siren-json
    test-invariant text time unordered-containers wai
  ];
